...
class EntityService:
    ...
    def __init__(self, entity_repository: EntityRepositoryInterface, query_interface: QueryInterface):
    ...
    async def update_entity_with_validation(self, entity_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def create_entity_relationship(self, entity_id: str, target_id: str, relationship_type: str, properties: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    async def search_entities_with_filters(self, search_term: str, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """Search entities with advanced filtering."""
        try:
            # Basic search
            entities = await self.query_interface.search_entities(
                search_term, 
                filters.get("entity_type") if filters else None
            )
    ...
    async def _validate_entity_updates(self, entity: Dict[str, Any], updates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Validate new values
        if "type" in updates and updates["type"] not in ["character", "location", "item", "concept"]:
            errors.append("Invalid entity type")
        
        return {"valid": len(errors) == 0, "errors": errors}
    
    async def _validate_relationship(self, entity: Dict[str, Any], target: Dict[str, Any], relationship_type: str) -> Dict[str, Any]:
    ...
    async def _apply_entity_filters(self, entities: List[Dict[str, Any]], filters: Dict[str, Any]) -> List[Dict[str, Any]]:
