...
from core.interfaces.persistence import QueryInterface, SceneRepositoryInterface
...
class NarrativeService:
    ...
    async def create_scene_with_context(
        self, scene_data: dict[str, Any], story_id: str
    ) -> dict[str, Any]:
    ...
    async def add_scene_participant(
        self, scene_id: str, entity_id: str, role: str, context: dict[str, Any] | None = None
    ) -> dict[str, Any]:
    ...
    async def transition_scene_status(
        self, scene_id: str, new_status: str, reason: str | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if success and reason:
                ...
                old_status = scene.get("status", "unknown")
                if isinstance(old_status, str):
                    await self._log_status_transition(scene_id, old_status, new_status, reason)
            return {"success": success}
        except Exception as e:
            return {"success": False, "errors": [str(e)]}

    async def validate_scene_continuity(
        self, scene_id: str, narrative_content: str
    ) -> dict[str, Any]:
        ...
        try:
            ...
            # Validate continuity
            continuity_issues = await self._check_continuity_issues(
                scene, story_context, narrative_content
            )
    ...
    async def get_scene_narrative_context(self, scene_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if story_id and isinstance(story_id, str) and isinstance(sequence_number, int):
                previous_scenes = await self._get_previous_scenes_context(story_id, sequence_number)
            else:
                previous_scenes = []
    ...
    async def _validate_scene_data(
        self, scene_data: dict[str, Any], story_id: str
    ) -> dict[str, Any]:
    ...
    async def _validate_participant_addition(
        self, scene_id: str, entity_id: str, role: str
    ) -> dict[str, Any]:
    ...
    async def _validate_status_transition(
        self, scene: dict[str, Any], new_status: str
    ) -> dict[str, Any]:
    ...
    async def _get_scene_details(self, scene_id: str) -> dict[str, Any] | None:
    ...
    async def _check_continuity_issues(
        self, scene: dict[str, Any], story_context: dict[str, Any], content: str
    ) -> list[str]:
    ...
    async def _get_previous_scenes_context(
        self, story_id: str, current_sequence: int
    ) -> list[dict[str, Any]]:
    ...
    async def _store_participation_context(
        self, scene_id: str, entity_id: str, context: dict[str, Any]
    ) -> None:
        """Store additional context about entity participation."""
        pass  # Placeholder

    async def _log_status_transition(
        self, scene_id: str, old_status: str, new_status: str, reason: str
    ) -> None:
