...
from core.services.narrative_agent_tools import NarrativeContentRecorder, NarrativeContextProvider
...
def build_enhanced_narrative_flow(tools: Any, config: dict | None = None):
    ...
    def narrative_council(state: dict[str, Any]) -> dict[str, Any]:
        ...
        council_input = {
            ...
            "scene_relationships": comprehensive_context.get(
                "scene_relationships", []
            ),  # From relations_effective_in_scene()
            "narrative_state": comprehensive_context.get("narrative_state"),  # From MongoDB
            "story_progression": story_timeline.get(
                "story_progression", []
            ),  # From scenes_in_story()
        ...
        council_decision = {
            ...
            "grounding_summary": _create_grounding_summary_real(
                comprehensive_context, story_timeline
            ),
        }
    ...
    def character_aware_narrator(state: dict[str, Any]) -> dict[str, Any]:
        ...
        for char_id, context in character_contexts.items():
            ...
            if entity_data:
                char_note_parts.append(
                    f"Entity: {entity_data.get('name', char_id)} ({entity_data.get('entity_type', 'unknown')})"
                )
...
"""
    ...
    def _analyze_character_impact_with_real_data(council_input: dict[str, Any]) -> dict[str, Any]:
        ...
        scene_relationships = council_input.get(
            "scene_relationships", []
        )  # From relations_effective_in_scene()
    ...
    def _analyze_plot_with_real_data(council_input: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(story_progression) > 0:
            plot_analysis["story_momentum"] = (
                "building" if len(story_progression) >= 3 else "starting"
            )

            # Get recent scenes
            recent_scenes = (
                story_progression[-2:] if len(story_progression) > 1 else story_progression
            )
    ...
    def _create_grounding_summary_real(
        comprehensive_context: dict[str, Any], story_timeline: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        summary = {
            ...
            "active_stories": [
                story.get("title", story.get("id", "Unknown")) for story in stories[:3]
            ],
            "main_characters": [
    ...
    def _analyze_relationships_in_content(
