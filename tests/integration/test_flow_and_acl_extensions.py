...
def test_recorder_dto_validation_error_on_bad_participants():
    ...
    bad = {
        "scene_id": "scene:1",
        "facts": [{"description": "bad participants shape", "participants": "not-a-list"}],
    }
...
def test_flow_executes_indexing_and_retrieval_actions():
    ...
    def indexing_tool(_ctx, *, llm=None, vector_collection, text_index, docs):
        ...
        return {
            "ok": True,
            "mode": "commit",
            "result": {"indexed": {"vectors": len(docs), "texts": len(docs)}},
        }
    ...
    tools_pkg = {
        ...
        "bootstrap_story_tool": lambda _ctx, **kw: {
            "refs": {"scene_id": "scene:x", "universe_id": "u1", "story_id": "st1"}
        },
...
def test_flow_injects_citations_into_narrative_and_sets_narrative_result():
        ...
        def act(self, _msgs):
            return json.dumps(
                [
                    {
                        "tool": "retrieval",
                        "args": {"query": "q", "vector_collection": "kb", "text_index": "kb"},
                    },
    ...
    tools_pkg = {
        ...
        "bootstrap_story_tool": lambda *_a, **_k: {
            "refs": {"scene_id": "s", "universe_id": "u", "story_id": "st"}
        },
